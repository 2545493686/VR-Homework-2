...
public class TouchToPlane : MonoBehaviour
{
    ...
    {
        Vector3 normalEndPoint = new Vector3(beganPos.x, beganPos.y, beganPos.z + 10);

        if (UpdateClipPlane(beganPos, exitPos, normalEndPoint))
        {
            ClipMesh();
        }
    }

    bool UpdateClipPlane(Vector3 touchBeganPoint, Vector3 touchEndPoint, Vector3 normalEndPoint)
    {
        if (Vector3.Distance(touchBeganPoint, touchEndPoint) == 0.0f)
        {
            return false;
        }
        ////转世界坐标
        ...
        //世界坐标转本地坐标
        Vector3 touchBeginPoint_local = transform.worldToLocalMatrix.MultiplyPoint(touchBeganPoint);
        Vector3 touchEndPoint_local = transform.worldToLocalMatrix.MultiplyPoint(touchEndPoint);
        Vector3 normalEngPoint_local = transform.worldToLocalMatrix.MultiplyPoint(normalEndPoint);
